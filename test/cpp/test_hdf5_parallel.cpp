/**
 * @file test_hdf5_parallel.cpp
 * @brief Parallel HDF5 + MPI smoke test for dftracer interception (C++)
 */
...
static int run_parallel_hdf5(const char* file_path, int rank, int nprocs) {
  ...
  const hsize_t local_elems = 16;
  const hsize_t global_elems = local_elems * static_cast<hsize_t>(nprocs);
  ...
  hid_t dset = H5Dcreate2(file, "parallel_dset_cpp", H5T_NATIVE_INT, filespace,
  ...
  hsize_t start[1] = {static_cast<hsize_t>(rank) * local_elems};
  hsize_t count[1] = {local_elems};
  if (H5Sselect_hyperslab(filespace, H5S_SELECT_SET, start, nullptr, count,
                          nullptr) < 0) {
    H5Sclose(memspace);
    H5Dclose(dset);
    H5Sclose(filespace);
  ...
  std::vector<int> wbuf(static_cast<size_t>(local_elems));
  std::vector<int> rbuf(static_cast<size_t>(local_elems), -1);
  for (size_t i = 0; i < wbuf.size(); ++i) {
    wbuf[i] = rank * 1000 + static_cast<int>(i);
  }

  if (H5Dwrite(dset, H5T_NATIVE_INT, memspace, filespace, dxpl, wbuf.data()) <
      0) {
    H5Pclose(dxpl);
    H5Sclose(memspace);
    H5Dclose(dset);
  ...
  if (H5Dread(dset, H5T_NATIVE_INT, memspace, filespace, dxpl, rbuf.data()) <
      0) {
    H5Pclose(dxpl);
    H5Sclose(memspace);
    H5Dclose(dset);
...
int main(int argc, char* argv[]) {
  ...
  char filename[1024];
  std::snprintf(filename, sizeof(filename), "%s/test_hdf5_parallel_cpp.h5",
                data_dir);

  DFTRACER_CPP_METADATA(meta, "hdf5_parallel_test", "hdf5_parallel_smoke_cpp");
